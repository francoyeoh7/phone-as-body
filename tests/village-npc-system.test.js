...
describe("VillageNpcSystem", () => {
  it("creates only the indoor grandmother actor and loads her without blocking", async () => {
    ...
    expect([...system.actors.keys()]).toEqual(["elowen"]);
    expect(system.actors.get("elowen").root.position.toArray()).toEqual([1.25, 0, 12.45]);
    expect(scene.getObjectByName("village-npcs")).toBe(system.root);
    await expect(system.load()).resolves.toEqual({ loaded: 0, fallback: 1 });
    expect(loader.loadAsync).toHaveBeenCalledTimes(1);
    expect(loader.loadAsync).toHaveBeenCalledWith("/assets/npcs/models/elowen-herbalist.glb");
    expect(system.snapshots()).toHaveLength(1);
  });
  ...
  it("marks one actor noticed and returns independent world snapshots", () => {
    ...
    expect(system.notice("elowen", new THREE.Vector3(0, 1.6, 0))).toBe(true);
    expect(system.actors.get("elowen").noticed).toBe(true);
    expect(system.notice("bram", new THREE.Vector3(0, 1.6, 0))).toBe(false);
    expect(system.snapshots()[0].aliases).toContain("Elowen");
  });
