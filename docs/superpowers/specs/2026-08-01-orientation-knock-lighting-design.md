...
- During a horizontal yaw turn, positive pitch up to 18 degrees relative to the pitch at turn onset is treated as wrist-coupled lift and suppressed. Compensation begins only when timestamped yaw reaches 30 degrees/second, its step is at least 1.25 times the simultaneous pitch step, and same-direction yaw accumulates 1.5 degrees. Downward pitch remains direct, upward intent beyond 18 degrees emits only the continuous excess, and the captured lift stays compensated after yaw stops so it cannot leak back as a delayed camera jump.
