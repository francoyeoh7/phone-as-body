...
- During a horizontal yaw turn, positive pitch up to 20 degrees relative to the pitch at turn onset is treated as wrist-coupled lift and suppressed. Compensation begins only when timestamped yaw reaches 30 degrees/second and its step is at least 1.25 times the simultaneous pitch step. Downward pitch remains direct, upward intent beyond 20 degrees emits only the continuous excess, and the captured lift stays compensated after yaw stops so it cannot leak back as a delayed camera jump.
